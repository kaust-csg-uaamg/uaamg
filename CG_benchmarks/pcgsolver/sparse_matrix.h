...
void multiply(const SparseMatrix<T> &matrix, const std::vector<T> &x, std::vector<T> &result)
{
   ...
   tbb::parallel_for(0, matrix.n, [&](int i) {
       result[i] = 0;
...
struct FixedSparseMatrix
{
   ...
   void construct_from_matrix(const SparseMatrix<T> &matrix)
   {
      ...
      tbb::parallel_scan(
          ...
          0/*identity*/,
          /*scan*/
          [&](const tbb::blocked_range<int>& r, size_t sum, bool is_final_scan) {
              size_t temp = sum;
              for (int i = r.begin(); i < r.end(); ++i) {
                  temp = temp + matrix.index[i].size();
                  if (is_final_scan) {
                      rowstart[i + 1] = temp;
                  }
     ...
     /* rowstart[0]=0;
	  ...
	  tbb::parallel_for(0, (int)n, [&](int i) {
		  int offset_begin = rowstart[i];
		  int offset_end = rowstart[i + 1];
          for (int j = offset_begin; j != offset_end; ++j) {
			  value[j] = matrix.value[i][j - offset_begin];
			  colindex[j] = matrix.index[i][j - offset_begin];
		  }
		  });
     /* unsigned int j=0;
      for(unsigned int i=0; i<n; ++i){
